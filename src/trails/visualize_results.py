...
def main():
    ...
    for iter1,file in enumerate(perc_files):
        ...
        error = df_isolated.groupby('frac_counter').std()['pct_isolated'].values*100
        y = df_isolated.groupby('frac_counter').mean()['pct_isolated'].values*100
        ...
        for iter2,ax in enumerate(axs.flatten()):
            ...
            if iter2 == 0:
                ...
                ax.set_ylabel('Percentage of trips isolated', fontsize=15, fontweight='bold')
                ax.set_xlabel('Percentage of network removed', fontsize=15, fontweight='bold')                                                                                                    
            else:
